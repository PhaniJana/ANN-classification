import streamlit as st
import numpy as np
import tensorflow as tf
from tensorflow.keras.models import load_model

from sklearn.preprocessing import StandardScaler, LabelEncoder, OneHotEncoder
import pandas as pd
import pickle

# Load the trained model
#Load the model
model=load_model('model.h5')

#Load Encoder and Scaler
with open('label_encoder.pkl','rb') as file:
    gender_encode=pickle.load(file)

with open('one_hot_encoder.pkl','rb') as file:
    geo_encode=pickle.load(file)

with open('scaler.pkl','rb') as file:
    scaling=pickle.load(file)    


## streamlit app
st.title('Customer Churn Prediction')

# User input
geography = st.selectbox('Geography', geo_encode.categories_[0])
gender = st.selectbox('Gender', gender_encode.classes_)
age = st.slider('Age', 18, 92)
balance = st.number_input('Balance')
credit_score = st.number_input('Credit Score')
estimated_salary = st.number_input('Estimated Salary')
tenure = st.slider('Tenure', 0, 10)
num_of_products = st.slider('Number of Products', 1, 4)
has_cr_card = st.selectbox('Has Credit Card', [0, 1])
is_active_member = st.selectbox('Is Active Member', [0, 1])


# Prepare the input data (include 'Geography' and encode as in prediction.ipynb)
input_dict = {
    'CreditScore': credit_score,
    'Geography': geography,
    'Gender': gender,
    'Age': age,
    'Tenure': tenure,
    'Balance': balance,
    'NumOfProducts': num_of_products,
    'HasCrCard': has_cr_card,
    'IsActiveMember': is_active_member,
    'EstimatedSalary': estimated_salary
}
input_df = pd.DataFrame([input_dict])

# Encode Gender
input_df['Gender'] = gender_encode.transform(input_df['Gender'])

# One-hot encode Geography
geo_encoded = geo_encode.transform(input_df[['Geography']])
geo_encoded_df = pd.DataFrame(geo_encoded.toarray(), columns=geo_encode.get_feature_names_out(['Geography']))

# Combine and drop original Geography
input_df = pd.concat([input_df.drop('Geography', axis=1), geo_encoded_df], axis=1)

# Scale the input data
input_data_scaled = scaling.transform(input_df)


# Predict churn
prediction = model.predict(input_data_scaled)
prediction_proba = prediction[0][0]

st.write(f'Churn Probability: {prediction_proba:.2f}')

if prediction_proba > 0.5:
    st.write('The customer is likely to churn.')
else:
    st.write('The customer is not likely to churn.')
